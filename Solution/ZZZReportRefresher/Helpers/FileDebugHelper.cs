...
{
    internal class FileDebugHelper
    {
        ...
        internal void LogUpdateReportsOutput(UpdateReportsOutput updateReportsOutput)
        {
            ...
            if (updateReportsOutput.ManagedException != null)
            {
                _epPlusHelper.AddNewContentRow(worksheetName, "ManagedException.InnerException.MessaggioPerUtente", updateReportsOutput.ManagedException.MessaggioPerUtente.ToString());
                _epPlusHelper.AddNewContentRow(worksheetName, "ManagedException.InnerException", updateReportsOutput.ManagedException.ToString());
                if (updateReportsOutput.ManagedException.InnerException != null)
                {
                    _epPlusHelper.AddNewContentRow(worksheetName, "ManagedException.InnerException", updateReportsOutput.ManagedException.InnerException.ToString());
                }
            }
        ...
        #region Fornitori
        internal void LogFornitoriCensiti(List<FornitoreCensito> listaFornitori, List<Reparto> repartiCensiti)
        {
            LogFornitoriCensiti(listaFornitori, repartiCensiti, WorkSheetNames.FornitoriCensiti);
        }
        internal void LogFornitoriAggiuntiDaInterfaccia(List<FornitoreCensito> listaFornitori, List<Reparto> repartiCensiti)
        {
            LogFornitoriCensiti(listaFornitori, repartiCensiti, WorkSheetNames.FornitoriAggiuntiDaInterfaccia);
        }
        internal void LogFornitoriAggiuntiDaListaDati(List<FornitoreCensito> listaFornitori, List<Reparto> repartiCensiti)
        {
            LogFornitoriCensiti(listaFornitori, repartiCensiti, WorkSheetNames.FornitoriAggiuntiDaListaDati);
        }

        private void LogFornitoriCensiti(List<FornitoreCensito> listaFornitoriCensiti, List<Reparto> repartiCensiti, string worksheetName)
        {
            if (_epPlusHelper == null) { return; }

            // riga intestazione
            _epPlusHelper.AddNewHeaderRow(worksheetName,
                "Sigla fornitore in Report",//#1
                "Categoria",//#2
                "Presente solo in ListaDati",//#3
                "Costo orario predefinito"//#4
                );
            // nomi reparto sulla riga di intestazione
            var colonnaPerReparto = 5;
            foreach (var reparto in repartiCensiti.OrderBy(_ => _.Nome))
            {
                _epPlusHelper.SetValue(worksheetName, 1, colonnaPerReparto++, reparto.Nome);
            }


            var rigaCorrente = 2;
            foreach (var fornitoreCensito in listaFornitoriCensiti.OrderBy(_ => _.SiglaInReport))
            {
                _epPlusHelper.SetValue(worksheetName, rigaCorrente, 1, fornitoreCensito.SiglaInReport);//#1
                _epPlusHelper.SetValue(worksheetName, rigaCorrente, 2, fornitoreCensito.Categoria);//#2
                _epPlusHelper.SetValue(worksheetName, rigaCorrente, 3, fornitoreCensito.PresenteSoloInListaDati);//#3
                _epPlusHelper.SetValue(worksheetName, rigaCorrente, 4, fornitoreCensito.CostoOrarioStandard);//#4

                colonnaPerReparto = 5;
                foreach (var reparto in repartiCensiti.OrderBy(_ => _.Nome))
                {
                    var costoOrarioPerReparto = fornitoreCensito.GetCostoOrarioPerReparto(reparto.Nome);
                    if (costoOrarioPerReparto != fornitoreCensito.CostoOrarioStandard)
                    {
                        _epPlusHelper.SetValue(worksheetName, rigaCorrente, colonnaPerReparto, costoOrarioPerReparto);
                    }
                    colonnaPerReparto++;
                }
                rigaCorrente++;
        ...
        internal void LogFornitoriNonCensitiInReport(List<FornitoreNonCensito> fornitoriConDatiMancanti)
        {
            if (_epPlusHelper == null) { return; }

            var worksheetName = WorkSheetNames.FornitoriNonCensitiInReport;

            // riga intestazione
            _epPlusHelper.AddNewHeaderRow(worksheetName,
                "Nome fornitore non censito in report",//#1
                "Foglio (reparto) in cui compare"//#2
                );

            foreach (var fornitoreNonCensitiInReport in fornitoriConDatiMancanti)
            {
                var testoPerFogliReparto = "";
                foreach (var foglioReparto in fornitoreNonCensitiInReport.FornitoriNonCensiti_InfoFoglioReparto)
                {
                    testoPerFogliReparto += $"{foglioReparto.Foglio} ({foglioReparto.Reparto}); ";
                }

                _epPlusHelper.AddNewContentRow(worksheetName,
                    fornitoreNonCensitiInReport.NomeSuController,//#1 
                    testoPerFogliReparto//#2
                    );
            }
        ...
        internal void LogSpesaSkippate(List<RigaSpeseSkippata> righeSpesaSkippate)
        {
            ...
            foreach (var rigaSpeseSkippata in righeSpesaSkippate)
            {
                _epPlusHelper.AddNewContentRow(worksheetName,
                   ...
                   rigaSpeseSkippata.Reparto,//#4
                   rigaSpeseSkippata.DatoNonValido//#5
                    );
            }
